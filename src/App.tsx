...
const App: React.FC = () => {
  ...
  const syncData = useCallback(async () => {
    ...
    try {
        ...
        const usersList = dbUsers.length > 0 ? dbUsers : [DEFAULT_ADMIN];
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem('expedi_session');
    if (saved) {
        try {
  ...
  const renderView = () => {
    if (!currentUser) {
        return <AuthScreen onLogin={handleLogin} users={allUsers} packages={packages} checkUserExists={(p) => allUsers.some(u => u.phone === p)} />;
    }
    ...
    switch(currentUser.role) {
        ...
        case UserRole.SENDER: return <SenderDashboard user={currentUser} packages={packages} allUsers={allUsers} notifications={notifications} pricingConfig={pricingConfig} onMarkNotifAsRead={DatabaseService.markNotificationRead} onCreatePackage={DatabaseService.savePackage} />;
        case UserRole.COURIER: return <CourierDashboard user={currentUser} packages={packages} notifications={notifications} lastGain={null} lastSync={Date.now()} isSyncing={isSyncing} onAcceptPackage={DatabaseService.updatePackageStatus} onUpdateStatus={DatabaseService.updatePackageStatus} onMarkNotifAsRead={DatabaseService.markNotificationRead} onClearNotifications={() => {}} onRecharge={() => {}} onCloseGainModal={() => {}} />;
